 ...
 */
function segenvita_woocommerce_styles( $enqueue_styles ) {

	$enqueue_styles[ genesis_get_theme_handle() . '-woocommerce-styles' ] = [
		'src'     => get_stylesheet_directory_uri() . '/lib/woocommerce/segenvita-woocommerce.css',
		'deps'    => '',
...
add_action( 'wp_enqueue_scripts', 'segenvita_woocommerce_css' );
/**
 ...
 */
function segenvita_woocommerce_css() {
	...
	$color_link   = get_theme_mod( 'segenvita_link_color', $appearance['default-colors']['link'] );
	$color_accent = get_theme_mod( 'segenvita_accent_color', $appearance['default-colors']['accent'] );
	...
	',
		$color_accent,
		segenvita_color_contrast( $color_accent )
	) : '';
